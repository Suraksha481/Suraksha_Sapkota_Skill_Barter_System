...
class RecommendationService
{
    ...
    public function suggestMatches(User $user): Collection
    {
        ...
        // 3. Score and Filter Recommendations
        $scoredRecommendations = $recommendations->map(function ($suggestedUser) use ($user, $wantedSkillModels, $wantedCategories) {
            $score = 0;
            $reasons = [];
            ...
            foreach ($offeredSkills as $offer) {
                ...
                foreach ($wantedSkillModels as $wanted) {
                    ...
                    $intersect = array_filter($intersect, function ($w) {
                                    return strlen($w) > 3; }
                                );
                        ...
                        $suggestedUser->match_score = $score;
                        $suggestedUser->match_reasons = array_unique($reasons);
                        $suggestedUser->is_recommendation = true;

                        return $suggestedUser;
                    });

        // 4. Return top suggestions
        return $scoredRecommendations->filter(function ($u) {
            return $u->match_score > 0;
